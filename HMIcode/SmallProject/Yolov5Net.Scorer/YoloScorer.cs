...
namespace Yolov5Net.Scorer
{
    ...
    public class YoloScorer<T> : IDisposable where T : YoloModel
    {
        ...
        private Tensor<float> ExtractPixels(Mat image)
        {
            ...
            //var aa =image.GetData().ToTensor<Byte>();
            //aa.
            //return image.GetData().ToTensor<float>();
            //var bitmap = (Bitmap)image;
            ...
            unsafe // speed up conversion by direct work with memory
            {
                ...
                //        Parallel.For(0, image.Width / 2, (x) =>
                //        {
                //            tensor[0, 0, y , x] = 0F; // r
                //            tensor[0, 1, y , x] = 0F; // g
                //            tensor[0, 2,  x] = 0F; // b
                ...
                Parallel.For(0, image.Height / 2, (y) =>
                {
                    //byte* row = (byte*)bitmapData.Scan0 + (y * bitmapData.Stride);

                    byte* row = (byte*)image.GetDataPointer(2 * y);
                    ...
                    Parallel.For(0, image.Width / 2, (x) =>
                    {
                        ...
                        //tensor[0, 0, y, x] = (float)image[y, x].Red / 255.0F; // r
                        //tensor[0, 1, y, x] = (float)image[y, x].Green / 255.0F; // g
                        //tensor[0, 2, y, x] = (float)image[y, x].Blue / 255.0F; // b

                        tensor[0, 0, y + 64, x] = row[2 * x * bytesPerPixel + 2] / 255.0F; // r
                        tensor[0, 1, y + 64, x] = row[2 * x * bytesPerPixel + 1] / 255.0F; // g
                        tensor[0, 2, y + 64, x] = row[2 * x * bytesPerPixel + 0] / 255.0F; // b
            ...
            sw.Stop();
            Debug.WriteLine("toTensor:" + sw.ElapsedMilliseconds);
        ...
        private Tensor<float> ExtractPixels2(Image image)
        {
            var bitmap = (Bitmap)image;
        ...
        private DenseTensor<float>[] Inference(Image image)
        {
            ...
            //resized.Save(@"C:\Users\pstech\Desktop\其他项目\焊接检测\resized.jpg");
        ...
        private List<YoloPrediction> ParseDetect(DenseTensor<float> output, Mat image)
        {
            var result = new ConcurrentBag<YoloPrediction>();
        ...
        private List<YoloPrediction> ParseSigmoid(DenseTensor<float>[] output, Mat image)
        {
            var result = new ConcurrentBag<YoloPrediction>();
        ...
        private List<YoloPrediction> ParseDetectyolo8(DenseTensor<float> output, Image image)
        {
            var result = new ConcurrentBag<YoloPrediction>();

            var (w, h) = (image.Width, image.Height); // image w and h
            var lenth = Math.Max(w, h);
            var (xGain, yGain) = ( (float)lenth / _model.Width,   (float)lenth / _model.Height); // x, y gains
            ...
            //{
            //    var index = 0;
            //    var score = output[0, 4, i];
            //    for (int k = 5; k < _model.Dimensions; k++)
            //    {
            //        if (output[0, k, i] > score)
            //        {
            //            score = output[0, k, i];
            //            index = k - 4;
            //        }
            ...
            //    if (score <= _model.MulConfidence) continue; // skip low mul_conf results

            //    float x1 = (float)(output[0, 0, i] - 0.5 * output[0, 2, i]) / gain;
            //    float y1 = (float)(output[0, 1, i] - 0.5 * output[0, 3, i]) / gain;
            //    float width = output[0, 2, i] / gain;
            //    float height = output[0, 3, i] / gain;

            //    YoloLabel label = _model.Labels[index];

            //    var prediction = new YoloPrediction(label, score)
            //    {
            //        Rectangle = new RectangleF(x1, y1, width, height)
            //    };
            ...
            Parallel.For(0, 8400, (i) =>
            {
                ...
                if (score <= _model.MulConfidence) return; // skip low mul_conf results

                float x1 = (float)(output[0, 0, i] - 0.5 * output[0, 2, i]) - xPad;
                float y1 = (float)(output[0, 1, i] - 0.5 * output[0, 3, i]) - yPad;
        ...
        private List<YoloPrediction> ParseOutput(DenseTensor<float>[] output, Mat image)
        {
            return _model.UseDetect ? ParseDetect(output[0], image) : ParseSigmoid(output, image);
        }
        private List<YoloPrediction> ParseOutputyolo8(DenseTensor<float>[] output, Image image)
        {
            return _model.UseDetect ? ParseDetectyolo8(output[0], image) : ParseSigmoid(output, image);
        }
        /// <summary>
        ...
        public List<YoloPrediction> Predict(Mat image)
        {
            return Supress(ParseOutput(Inference(image), image));
        }
        public List<YoloPrediction> Predictyolo8(Image image)
        {
            return Supress(ParseOutputyolo8(Inference(image), image));
        }
        /// <summary>
        ...
        /// </summary>
        /// <returns></returns>
        public List<YoloLabel> GetLabels()
        {
            if (_model != null)
            {
